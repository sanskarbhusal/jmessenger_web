...
export default class ChatBubble extends React.Component<Props, State> {
  ...
  render() {
    ...
    return (
      <div className="sm:w-[45%] sm:group sm:self-end sm:mt-10 sm:mb-10 sm:mr-10 sm:bg-white sm:rounded-xl">
        <div className="w-full h-full flex flex-col bg-custom-blue/10 sm:border border-custom-blue/30 rounded-xl selection:bg-custom-blue/90 selection:text-white">
          ...
          <div className="flex flex-row justify-end items-end gap-1 pr-1">
            <div className="flex flex-row items-center font-medium text-xs text-gray-500">{time}</div>
            <DoubleCheckIcon className="text-custom-blue/90 group-hover:text-gray-400 font-bold text-md h-fit w-fit " />
          </div>
        </div>
      </div>
    )
